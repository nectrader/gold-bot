...
def run():
    ...
    isBuy=dir4h=="buy"
    sl_pips=max(80,min(150,round(a/0.10*0.8)))
    tp1_pips=sl_pips
    ...
    msg=(f"{d}\n\n"
         f"Prix d'entrée ➡️ {entry} ( exécution immédiate )\n\n"
    ...
    send(msg)
    print(f"Signal envoyé : {d}")

send("🤖 Gold Bot démarré — surveillance active 24h/24 🥇")
while True:
    try:
